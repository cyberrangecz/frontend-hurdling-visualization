...
export class TraineeMapper {
  static fromDTOs(
    dtos: TraineeDTO[],
    progressTraineesDTOs: TraineeProgressDTO[]
  ): Trainee[] {
    const result = dtos.map((dto, index) =>
      TraineeMapper.fromDTO(dto, progressTraineesDTOs[index], index)
    );
    return result;
  }
  ...
  static fromDTO(
    dto: TraineeDTO,
    progressTraineeDTO: TraineeProgressDTO,
    index: number
  ): Trainee {
    ...
    result.trainingRunId = progressTraineeDTO
      ? progressTraineeDTO.training_run_id
      : null;
    return result;
  }
}
