...
export abstract class VisualizationsDataService {
  protected visualizationDataSubject$: ReplaySubject<VisualizationData> =
    new ReplaySubject();
  protected commandLineDataSubject$: ReplaySubject<CommandLineEntry[]> =
    new ReplaySubject();
  ...
  commandLineData$: Observable<CommandLineEntry[]> =
    this.commandLineDataSubject$
      .asObservable()
      .pipe(filter((vd) => vd !== undefined && vd !== null));

  abstract getData(trainingInstanceId: number): Observable<VisualizationData>;

  abstract getCommandLineData(
