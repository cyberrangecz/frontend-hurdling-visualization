...
export class ProgressComponent implements OnChanges, OnInit, AfterViewInit {
  ...
  private transformTimeStamp = false; //we need to remember a transform fire timestamp and check it, otherwise the brush update will trigger an infinite loop (this issue emerged from d3v7)
  ...
  sortTrainees() {
      if (this.sortType == 'name') {
          ...
          this.visualizationData.traineeProgress = this.visualizationData.traineeProgress.sort(
              (a,b) => this.sortReverse ?
 ...
 getTraineeTime(progress: TraineeProgress) {
  ...
  updateDisplayedTrainees(): TraineeProgress[] {
    ...
    let visibleTrainees: TraineeProgress[] = [];
 ...
 updateWhenEmpty(){ //traineeData: TraineeProgress[]) {
    ...
    this.svg.select('.progress-chart-container .timeline')
  ...
  initZoom(): void {
    ...
    this.svg.select('.progress-chart-container')
  ...
  initHeaders() {
    const currentTimePos =
        ...
        this.timeIndication * 0.55 : this.timeIndication);

    this.d3.select('.trainee-name-header')
        .attr('x', 0)
        .style('left', (this.margin.left - 80) + 'px');

    this.d3.select('#progress-elapsed-time')
        .style('left', (this.margin.left + currentTimePos) + 'px');

    this.d3.select('.trainee-time-header')
  ...
  zoomed(event, reset: boolean = false): void {
    ...
    if (this.brush != undefined && event && event.sourceEvent && this.transformTimeStamp !== event.sourceEvent.timeStamp) {
      ...
      this.d3
          ...
          .call(
              this.brush.move,
              this.xScale.range().map(this.zoomTransform.invertX, this.zoomTransform)
          );
    }
  }
  ...
  transformChart(transform: ZoomTransform) {
    ...
    this.d3.select('#progress-elapsed-time')
        ...
        .style('opacity', () => {
          ...
          newPosition < this.xScale(this.visualizationData.startTime) ? '0' : '1';
        });
    ...
    this.svg
        .selectAll('defs')
        .selectAll('pattern')
        .selectAll('rect').attr('width', 3 / transform.k);
  }
  ...
  brushed(event): void {
    const newPosition =  this.xScale(this.visualizationData.currentTime) * this.zoomTransform.k + this.zoomTransform.x;
    const moveTimeText = newPosition < this.timeIndication  ? this.timeIndication * 0.55 : this.timeIndication;
    this.d3.select('#progress-elapsed-time')
        .style('left', this.margin.left + newPosition - moveTimeText + 'px');
    ...
    const subBars = this.d3.selectAll('.context .row-container')
    ...
    subBars
        ...
        .each((d, i, nodes) => {
          this.d3.select(nodes[i]).selectAll('rect')
              .attr('height', this.brushYScale.bandwidth())
  ...
  setWidth() {
    ...
    ) * this.approxFontWidth + 20; // 20 = additional margin before name

    // set right side for trainee times by the duration (only times or with days?)
    this.margin.right = this.getTimeString(
        (this.visualizationData.currentTime - this.visualizationData.startTime)
    ).length * this.approxFontWidth + 20; // 20 = additional margin after time

    this.width = (this.d3.select('.progress-container').node() as HTMLElement)
        .offsetWidth - this.margin.left - this.margin.right;
  }

  setHeight() {
    this.chartHeight = this.appConfig.traineeRowHeight * this.filteredTrainees.length;
  ...
  drawTimeline() {
    ...
    this.svg.selectAll('.progress-chart-container .progress-row-container')
        .append('line')
        .attr('class', 'timeline')
  ...
  initProgressRowsContainer(): void {
    ...
    clip
      .append('g')
      .attr('class', 'progress-row-container');

    clip
      .append('g')
      .attr('class', 'progress-row-events');
  }
  ...
  initProgressRow(): void {
      ...
      .classed('highlighted', (d: TraineeProgress) => this.highlightedTraineeRefId == d.userRefId)
      .on('mouseover', (event, d) => {
          this.d3.select(event.currentTarget).classed('highlighted', true);
          const levelId = +this.d3.select(event.target).attr('level-id');
          const trainee = this.visualizationData.trainees
              .filter(trainee => trainee.userRefId == d.userRefId)[0];
          const level = this.visualizationData.levels
              .filter(level => level.id == levelId)[0];
          ...
          const runBox = document.getElementById('level-' + levelId + '-run-' + d.trainingRunId).getBoundingClientRect();
          // furthermore, in the case of zoom, we need to place the tooltip into the visible chart area
          const progressBox = document.querySelector('.progress-chart-container .zoom-listener-rect').getBoundingClientRect();
  ...
  drawFinished() {
    const finished = this.svg
          ...
          .data((traineeProgress: TraineeProgress) => traineeProgress.displayRun && traineeProgress.levels);
    ...
    finished
      ...
      .attr(
        ...
        (traineeLevel: TraineeLevel) =>
          this.xScale(traineeLevel.endTime) - this.xScale(traineeLevel.startTime)
      )
  ...
  drawActiveEstimation(): void {
    ...
    activeEstimations
      ...
      .attr('x', (traineeLevel: TraineeLevel) => this.xScale(traineeLevel.startTime))
      .attr('y', (_, i, j) =>
        this.yScale((this.d3.select(j[i].parentNode).data() as any)[0].trainingRunId)
      )
      .attr(
        'width',
        (traineeLevel: TraineeLevel) => {
      ...
      .attr('fill', (traineeLevel: TraineeLevel) =>
  ...
  drawActiveElapsed(): void {
    const activeElapsed = this.svg
      ...
      .data((traineeProgress: TraineeProgress) =>
          traineeProgress.displayRun && traineeProgress.levels.filter(
          (traineeLevel: TraineeLevel) => traineeLevel.state == 'RUNNING'
        )
      );
    activeElapsed
  ...
  getActiveLevelColor(traineeLevel: TraineeLevel) {
    ...
    let currentLevelTime = this.visualizationData.currentTime - traineeLevel.startTime;
        ...
        'width', (level: Level) =>
          this.xScale(
            ...
            (level.estimatedDuration > 0 ? level.estimatedDuration : conf.defaultEstimatedTime) * 60 +
              this.visualizationData.startTime) - this.xScale(this.visualizationData.startTime)
      )
  ...
  setHighlightedTrainee(trainee: Trainee) {
    ...
    const training = this.visualizationData.traineeProgress.find(p => p.userRefId == this.highlightedTraineeRefId);

    if (!training) return;
    rows.select('.run-' + training.trainingRunId + '.row-container')
  ...
  groupEvents(transformScale: number = 1): void {
    const eventIconSize: number = this.appConfig.eventProps.eventIconSize;
    let runsWithEvents = JSON.parse(JSON.stringify(this.visualizationData.traineeProgress)).map(run => {
    ...
    runsWithEvents.forEach((run) => {
      ...
      run.levels.forEach((level) => {
        ...
        level.events.forEach((ev, i) => {
          ...
          ev.position = (
              this.xScale(ev.timestamp) - this.xScale(this.visualizationData.startTime)
          ) * transformScale;
  ...
  createEvents(transformScale: number = 1) {
    ...
    const eventLayers: any = this.svg.select('.progress-row-events')
        ...
        .style('transform', (d) =>
           'translate(' + this.xScale(this.visualizationData.startTime) + 'px, ' +
             this.yScale(d.trainingRunId) + 'px)'
        )
        .on('mouseover', (d) => {
          // preserve the team highlight
          this.svg.select('.run-' + d.trainingRunId)
              .classed('highlighted', true);
        })
        .on('mouseout', (d) => {
  ...
  private addTraineeName(traineeRuns: TraineeProgress[], traineeInfo: Trainee[]) {
  ...
  private addTraineeAvatar(traineeRuns: TraineeProgress[], traineeInfo: Trainee[]) {
    ...
    this.svg.select('.progress-chart-container')
        ...
        .append('image')
        .attr('trainee-id', (d) => d.userRefId)
        .attr('xlink:href', (d) => 'data:image/png;base64,'
  ...
  public getTimeString(seconds: number): string {const days: number = Math.floor(seconds / 86400);
