...
export class LegendComponent implements OnInit {
  ...
  updateLevelColor() {
    this.activeLevelColorIndex =
      (this.activeLevelColorIndex + 1) %
      this.appConfig.levelsColorEstimates.length;
    const newColor =
      this.appConfig.levelsColorEstimates[this.activeLevelColorIndex];
    this.levelColors.active = this.d3.hsl(newColor).brighter(1.2).toString();

    this.finishedLevelColorIndex =
      (this.finishedLevelColorIndex + 1) % this.appConfig.trainingColors.length;
    this.levelColors.finished =
      this.appConfig.trainingColors[this.finishedLevelColorIndex];
  }
