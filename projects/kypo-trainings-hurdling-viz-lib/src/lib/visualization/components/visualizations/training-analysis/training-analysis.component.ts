...
import { AfterViewInit, Component, EventEmitter, Input, OnChanges, OnDestroy, Output, ViewEncapsulation } from "@angular/core";
    ...
    implements OnChanges, OnDestroy, AfterViewInit
{
    ...
    getTotalTime(traineeId: number): number {
        ...
        if (this.getStateForTrainee(traineeId) == "FINISHED") {
            ...
            return (
                ...
                traineeStartTime +
                this.trainingDataSet.find((trainee) => trainee.traineeId == traineeId)
                    ?.start
            );
        }
    ...
    getLongestEstimate(): number {
        ...
        this.visualizationData.traineeProgress.forEach((traineeProgress) => {
            let remainingTime = traineeProgress.levels
                .map((traineeLevel, i) => {
    ...
    drawChartBase(baseConfig: BaseConfig): void {
        ...
        this.planDomain = Math.max(
            estimatedTime,
            d3.max(layers[layers.length - 1], (d: number[]): number => {return d[1];})
        );
    ...
    initializeScales(planData) {
        ...
        const yDomain = planData.teams.map(
            (d: GenericObject): string => d.traineeName
        );
    ...
    createEvents({trainingData, eventShapePaths, groupCircleWidth, eventIconWidth}) {
        ...
        eventLayers
            ...
            .attr("fill", (d: GenericObject, i, nodes): string => {
                ...
                let colorIndex: number = +d.level - 1; // in final overview is no first transparent column for start
            ...
            .attr("transform", (group: GenericObject, i: number, nodes): string => {
                ...
                let y = this.yScale(teamStruct.traineeName) +
        ...
        eventLayers
            ...
            .attr("y", (d: GenericObject, i: number, nodes): string => {
                const teamStruct: DataEntry = <DataEntry>(d3.select(nodes[i].parentNode).datum()),
                    y = this.yScale(teamStruct.traineeName) +
                        this.yScale.bandwidth() * 0.5 +
                        groupCircleWidth / 5;
                return y.toString();
            })
    ...
    private addTraineeName(teamDataLayer, trainingData: TrainingData) {
        teamDataLayer
    ...
    onMouseWheelDown($event: any) {
        if (this.zoomValue > 1) {
            const newZoomValue = Math.max(
                1, this.zoomValue - this.appConfig.zoomStep),
                scale = newZoomValue / this.zoomValue,
                dx = (-$event.left + this.panValue) * scale + $event.left - this.panValue;
            this.zoomValue = newZoomValue;
            this.drawChart();
    ...
    getEventShapePaths() {
        return { ...this.appConfig.shapes, ...this.appConfig.eventProps.eventShapes }
    }
