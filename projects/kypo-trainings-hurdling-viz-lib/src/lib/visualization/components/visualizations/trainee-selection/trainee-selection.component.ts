...
export class TraineeSelectionComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    this.traineeSelectData.forEach(trainee => {
      ...
      if(trainee.isSelected == null && !this.checkIfActive(trainee.trainee)) {
        ...
        {trainee.isSelected = this.filteredTrainees.find(p => p.userRefId === trainee.trainee.userRefId) ? true : false;}
      trainee.warnings = {
  ...
  setTraineeColumnDistribution(): void {
    ...
    this.numberOfColumns = this.visualizationData.trainees.length <= this.maxNumOfColumns ?
  ...
  toggleTrainee(trainee: Trainee): void {
    this.traineeSelectData.find(pp => pp.trainee.userRefId == trainee.userRefId).isSelected =
      !this.traineeSelectData.find(pp => pp.trainee.userRefId == trainee.userRefId).isSelected;
    this.filteredTraineesChange.emit(this.traineeSelectData.filter(pp => pp.isSelected == true).map(pp => pp.trainee));
  }
  ...
  checkIfActive(trainee: Trainee): boolean {
    return this.visualizationData.traineeProgress.find(traineeProgress => traineeProgress.userRefId == trainee.userRefId) ? true : false;
  }
  ...
  buildTraineeTooltip(trainee): string {
    ...
    if(this.checkIfActive(trainee))
      tooltipText += this.getCurrentTraineeLevel(trainee) ? 'is in level: ' + this.getCurrentLevel(trainee).title : ' has finished';
    return tooltipText;
  }

  showTooltip(innerText, event): void {
    const tooltip = this.d3
        .select('.vis-participant-grid .kypo-viz-hurdling-trainee-tooltip');
  ...
  checkLateTrainee(trainee: Trainee): boolean {
    if (!this.checkIfActive(trainee))
    return false;
    if (!this.getCurrentTraineeLevel(trainee))
      return false;
    if (this.getCurrentLevel(trainee).estimatedDuration == 0)
      return false;
  ...
  checkOutOfHints(trainee: Trainee): boolean {
    const levelHints = this.visualizationData.levels.find(level => level.id == this.getCurrentTraineeLevel(trainee)?.id)?.hints;
    if(!levelHints || levelHints.length == 0) {
      return false;
    }
    const levelHintsTaken = this.getCurrentTraineeLevel(trainee).hintsTaken == null ? [] : this.getCurrentTraineeLevel(trainee).hintsTaken;
  ...
  getCurrentTraineeLevel(trainee: Trainee): TraineeLevel {
    return this.visualizationData.traineeProgress
      .find(p => p.userRefId == trainee.userRefId)?.levels
