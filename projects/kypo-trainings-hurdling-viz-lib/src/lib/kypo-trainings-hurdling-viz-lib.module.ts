...
export class KypoTrainingsHurdlingVizLibModule {
  constructor(
    @Optional() @SkipSelf() parentModule: KypoTrainingsHurdlingVizLibModule
  ) {
    if (parentModule) {
      throw new Error(
  ...
  static forRoot(
    config: HurdlingVisualizationConfig
  ): ModuleWithProviders<KypoTrainingsHurdlingVizLibModule> {
    return {
