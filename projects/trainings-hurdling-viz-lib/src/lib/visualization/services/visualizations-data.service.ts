...
export abstract class VisualizationsDataService {
    protected visualizationDataSubject$: ReplaySubject<HurdlingVisualizationData> = new ReplaySubject();
    protected commandLineDataSubject$: ReplaySubject<CommandLineEntry[]> = new ReplaySubject();

    visualizationData$: Observable<HurdlingVisualizationData> = this.visualizationDataSubject$
        .asObservable()
    ...
    abstract getData(trainingInstanceId: number): Observable<HurdlingVisualizationData>;
