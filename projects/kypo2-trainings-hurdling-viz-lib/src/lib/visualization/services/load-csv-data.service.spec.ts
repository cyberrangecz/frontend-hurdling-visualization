...
describe('LoadCsvDataService', () => {
  ...
  const pngFile = new File([''], 'picture.png', {type: 'image/png'}),
  ...
  it('should return error for a non valid CSV file', async(() => {
      service.getGameAndPlanData(invalidCsvFile, []).subscribe(() => {
        ...
        expect(error).toEqual('The provided CSV file could not be parsed. Required format: ' +
            'team, absolute time (YYYY-MM-DD HH:mm:ss), elapsed time (HH:mm:ss), level, event.');
      });
  }));

  it('should return game and plan data for a valid CSV file', async(() => {
      service.getGameAndPlanData(validCsvFile, []).subscribe((data) => {
