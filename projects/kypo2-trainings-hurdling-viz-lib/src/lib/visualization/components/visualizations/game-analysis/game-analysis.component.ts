...
export class GameAnalysisComponent implements OnChanges, OnDestroy, OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void  {
    ...
    this.subscribeHighlightPlayer();
  }

  setData(){
    this.visualizationData.startTime = this.getFirstEventTimestamp();
  ...
  getTime(): number {
    const allFinished = this.visualizationData.playerProgress
      ...
      .length == this.visualizationData.playerProgress.length;

    if(allFinished) {
      return this.getLatestEventTimestamp() - this.visualizationData.startTime;
    }
    else {
      return this.visualizationData.currentTime - this.visualizationData.startTime;
    }
  } 
  ...
  populateGameDataset(): GameDataEntry[]{
    ...
    this.visualizationData.playerProgress.forEach(playerProgress => {
      ...
      gameDataEntry['start']=(this.getFirstLevelTimestamp(playerProgress.userRefId)-this.visualizationData.startTime);
  ...
  getStateForPlayer(playerId: number): string {
    const levelsFinishedCount = this.getPlayerLevels(playerId).filter(playerLevel => playerLevel.state == "FINISHED").length;
    return levelsFinishedCount == this.levels.length ? "FINISHED" : 'level'+(levelsFinishedCount+1);
  }

  getTotalTime(playerId: number): number {
    const playerStartTime = (this.getFirstLevelTimestamp(playerId)-this.visualizationData.startTime);
  ...
  getPlayerLevels(playerId: number): PlayerLevel[] {
    return this.visualizationData.playerProgress
      .find(player => player.userRefId == playerId)
      .levels;
  }

  getPlayerData(playerId: number) {
    return this.visualizationData.players.find(player => player.userRefId == playerId);
  }
  ...
  private getLevelKeys(): string[] {
    ...
    if(this.view === View.Progress)
      levelKeys = ['start'].concat(levelKeys);
    return levelKeys;
  }
  ...
  getXAxisTickInterval(): number {
    ...
    if (this.wrapperWidth > 500 && this.wrapperWidth < 1200) { interval = 600; }
    ...
    this.planSegments = planLayers
      ...
      .style(
        ...
        (d: GenericObject): string =>
          {   
            return 'url(#diagonalHatch-' + this.getSegmentColor(
              d.data.playerName,
              Math.floor(++index / displayedParticipants)) + ')'
          }
      )
      .attr('y', (d: GenericObject): number => this.yScale(String(d.data.playerName)))
    ...
    layer
      ...
      .attr('width', (d: GenericObject, i: number, nodes) => {
            ...
            .style('opacity', () =>
              this.sortLevel !== 0 &&
                (typeof data['level' + this.sortLevel] === 'undefined' &&
                  currentState !== 'level' + this.sortLevel) ? 0.3 : 1
            );
        }
        ...
        if (typeof currentLevelData !== 'undefined') {
          ...
          if (this.view === View.Overview) finalWidth = xScale(this.levelsTimePlan[levelIndex]);
  ...
  createEvents({ gamedata, eventShapePaths, groupCircleWidth, eventIconWidth }) {
    ...
    eventLayers
      ...
      .attr('transform', (group: GenericObject, i: number, nodes): string => {
        ...
        let y = this.yScale(teamStruct.playerName) + this.yScale.bandwidth() * 0.5 - iconWidth / 2;
    ...
    if (append) {
      dataLayer
          ...
          .text((d: GenericObject): string =>
            (name === 'time' && !isNaN(d.totalTime)) ? this.getTimeString(d.totalTime) :
  ...
  onSortValueChange(sortType: string, sortReverse: boolean, levelIndex?: number): void {
    ...
    if (this.runsToCompare.length > 0) {
      ...
      this.d3
        .selectAll('.data text')
        .filter((data: any) => this.runsToCompare.some(run => run.id === data.playerName))
        .classed('data-hover', true);
    }
  }
  ...
  private subscribeHighlightPlayer() {
    this.highlightSubscription = this.playerSelectionService.getHighlightedPlayer().subscribe(player => {
      if (player !== null) {
        this.participants.forEach(item => {
          if (item.id === player.id) {
            this.highlightGivenPlayer(item.name);
          } else {
            this.unhighlightGivenPlayer(item.name);
          }
        });
      } else {
        this.participants.forEach(item => {
  ...
  highlightGivenPlayer(playerId: string): void {
    ...
    this.d3
        .selectAll('.data text')
        .filter((data: any) => data.id.toString() === playerId)
        .classed('data-hover', true);
    this.d3
      .selectAll('.game .game-layer rect')
      .filter((data: any) => data.data.id.toString() === playerId)
      .classed('data-hover', true);
  }
  ...
  preserveHighlightedPlayer(playerId: string): void {
    if (this.runsToCompare.some(run => run.id === playerId)) {
      this.runsToCompare = this.runsToCompare.filter(item => item.id !== playerId);
    } else {
  ...
  onFilterChange() {
    this.d3.selectAll('.events-row path.event')
        .style('display', group => this.setEventGroupVisibility(group));

    this.d3.selectAll('.events-row text.event-number')
        .style('display', group => this.setEventGroupVisibility(group));
  }
