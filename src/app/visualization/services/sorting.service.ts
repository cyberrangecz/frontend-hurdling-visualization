...
export class SortingService {
  ...
  constructor(d3service: D3Service) {
	...
	sort(gamedataset: GenericObject[], sortReverse: boolean, sortType: string, sortLevel: number, view, levels): GenericObject[] {
		this.view = view;
		this.levels = levels;
	...
	sortByLevelTime(gamedataset: GenericObject[], level: number, order: Order): GenericObject[] {
		...
		if (typeof gamedataset !== 'undefined') {
			...
			sorted.sort(function(teamA: GenericObject, teamB: GenericObject): number {
				...
				const lastLevelIndex: number = (this.lastLevelIndex === 2) ? level - 1 : level;
