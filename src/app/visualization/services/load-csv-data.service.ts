...
export class LoadCsvDataService {
	...
	public getGameAndPlanData(file: File, levelsTimePlan: number[], endInPercents: number = 100) {
		...
		const promise: Promise<any> = new Promise(function (resolve, reject) {
			...
			this.papa.parse(file, {
				...
				step: function (row): void {
					...
					if ((index === 0) || (d[4] === 'Game started' && parseInt(d[3]) === 1 && startTime > timestamp)) {
						startTime = timestamp;
					}
				...
				complete: function (): void {
					if (rawData.length === 0) {
						...
						reject('The provided CSV file could not be parsed. Required format: team, absolute time (YYYY-MM-DD HH:mm:ss), elapsed time (HH:mm:ss), level, event.');
					} else {
						const result: any = this.processCSVData(rawData, startTime, endInPercents);
						resolve(result);
					}
				}.bind(this)
			});
	...
	private processCSVData(rawData: DataEntry[], startTime: number, endInPercents: number = 100): any {
		...
		let levelCount: number = Math.max.apply(Math, rawData.map(function (d) { return d.level; }));
		...
		rawData.forEach(function (d: DataEntry, index: number): void {
			...
			while ((gamedataset[teamIndex]['level' + nextLevel] !== undefined) && (nextLevel <= levels.length)) {
			...
			switch (d.event) {
				case this.eventTypes.gameStart:
					...
					if (d.level === 1) {
						...
						gamedataset[teamIndex]['start'] = Math.max(0, d.timestamp - startTime);
					}
					break;
			...
			if (levelFinished) {
				...
				if (typeof gamedataset[teamIndex][levelKey] === 'undefined' || gamedataset[teamIndex][levelKey] < d.time) {
		...
		plandataset.forEach(function (team: GenericObject): void {
			let levelIndex = 0;
			levels.forEach(function (level): void {
				const timePlan: number = (levelsTimePlan[levelIndex]) ? levelsTimePlan[levelIndex] : levelTimePlan;
		...
		gamedataset.forEach(function (team: GenericObject): void {
			...
			if (typeof team[lastLevelKey] === 'undefined') team['totalTime'] = time - startTime - team['start']; // adjust total time
