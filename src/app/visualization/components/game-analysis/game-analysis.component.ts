...
import {
	LoadDataService
} from '../../services/load-data.service';
import {
	LoadCsvDataService
} from '../../services/load-csv-data.service';
...
import {
	DataEntry
} from '../../models/data-entry';
import {
	GameConfig
} from '../../models/game-config';
import {
	PlanConfig
} from '../../models/plan-config';
import {
	BaseConfig
} from '../../models/base-config';
import {
	Data
} from '../../models/data';
import {
...
import {
	View
} from '../../models/view.enum';
import {
	DataSource
} from '../../models/data-source.enum';

import {
	GenericObject
} from '../../models/generic-object.type';
import {
	NumericObject
} from '../../models/numeric-object.type';
...
export class GameAnalysisComponent implements OnInit {
	...
	constructor(
		...
		d3Service: D3Service, private loadDataService: LoadDataService,
	...
	drawPlan(planConfig: PlanConfig): void {
		...
		const pattern = defs.selectAll('pattern')
			...
			.attr('id', function (d: GenericObject, i: string): string {
		...
		this.bounds = this.gameChart.append('g')
			.attr('class', 'bounds');
		if (this.view === View.overview) {
			const boundGroups = this.bounds.selectAll('.bounds-layer')
				.data(layers)
				.enter().append('g')
			...
			this.boundSegments = boundGroups.selectAll('rect.plan-bound')
				...
				.attr('x', function (d: GenericObject): string {
					return ( < number > this.xScale(d[1]) - boundWidth).toString();
				}.bind(this))
				.attr('height', this.yScale.bandwidth())
	...
	drawGame(gameConfig: GameConfig): void {
		...
		this.gameDomain = Math.max(this.time, d3.max(layers[layers.length - 1], function (d: number[]): number {
			return d['data'].totalTime;
		}));
	...
	addDataColumns(dataColumns: GenericObject, gamedata: GameData) {
		...
		const teamData: any = d3.select('#' + dataColumns['team']).append('svg')
			.attr('height', this.wrapperHeight);
		const teamDataLayer: any = teamData.append('g')
			.attr('class', 'data');

		const teams: any = teamDataLayer.selectAll('text.data-team')
			.data(gamedata.teams)
			.enter().append('text')
			.text(function (d: GenericObject): string {
				return d.team;
			}.bind(this))
			.attr('y', function (d: GenericObject): string {
				return this.yScale(d.team) + this.yScale.bandwidth() * 0.6 + this.padding.top;
			}.bind(this))
			.attr('x', 130)
			.style('text-anchor', 'end');


		const timeData: any = d3.select('#' + dataColumns['time']).append('svg')
			.attr('height', this.wrapperHeight);
		const timeDataLayer: any = timeData.append('g')
			.attr('class', 'data');

		const times: any = timeDataLayer.selectAll('text.data-time')
			.data(gamedata.teams)
			.enter().append('text')
			.text(function (d: GenericObject): string {
				return !isNaN(d.totalTime) ? this.getTimeString(d.totalTime) : '';
			}.bind(this))
			.attr('y', function (d: GenericObject): string {
				return this.yScale(d.team) + this.yScale.bandwidth() * 0.6 + this.padding.top;
			}.bind(this))
	...
	onMouseWheelUp($event: any) {
		if (this.zoomValue < this.config.maxZoomValue) {
			const newZoomValue = Math.min(this.config.maxZoomValue, this.zoomValue + this.config.zoomStep),
